...
df <- df %>%
  mutate(instrucao = recode(instrucao,
...
saveRDS(df, file = paste(input_dir, "/rdd_data.rds", sep = ""))
...
covsZ = cbind(df$state.d, df$idade)
poli = 2
...
teste <- modelsummary(models,
  ...
  tab_spanner(label = "(3)", columns = 6:7) %>% 
  tab_spanner(label = "(4)", columns = 8:9) %>% 
  tab_spanner(label = "Linear", columns = 2:5) %>% 
  tab_spanner(label = "Quadratic", columns = 6:9)
